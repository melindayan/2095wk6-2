...
app.post('/addtask', function (req,res){
    ...
    db.collection('lab5').insertOne({taskId: taskID, taskName:req.body.taskName, assignTo:req.body.assignTo, taskDueDate: new Date(req.body.taskDueDate), 
...
app.get('/listtasks', function(req,res){
    ...
    db.collection('lab5').find(query).toArray(function (err, data) {
...
app.post('/deletebyid', function (req,res){
    ...
    db.collection('lab5').deleteOne(filter);
    res.redirect('/listtasks');
});
...
app.post('/deletecompletedtasks', function (req,res){
    ...
    db.collection('lab5').deleteMany(filter);
    res.redirect('/listtasks');
});

app.get('/deletedueandcompletedtasks', function (req,res){
    res.sendFile(viewsPath+"deletedueandcompletedtasks.html");
});


app.post('/deletedueandcompletedtasks', function (req,res){
    let filter = {$and: [{taskDueDate: { $lt: new Date() } },{taskStatus: {$eq: 'Complete'}} ] } ;
    db.collection('lab5').deleteMany(filter);
    res.redirect('/listtasks');
});
...
app.post('/updatebyid', function (req,res){
    ...
    db.collection('lab5').updateOne(filter, theUpdate);
    res.redirect('/listtasks');
});
